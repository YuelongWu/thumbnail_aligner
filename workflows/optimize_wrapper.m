function [Ms, cost_history] = optimize_wrapper(imglist, ptpairlist, fixed_sec, options, maskwdir, maskrdir, meshdir, z_wt, M0t)
    Nimg = numel(imglist);
    if Nimg == 0
        Ms = cell(0,1);
        return
    ...
    options = utils.set_default(options, 'post_translate_filter_thresh', 0);
    ...
    options = utils.set_default(options, 'Nsoft', 0);
    ...
    mt0 = options.params.intra_section.params.multiplier;
    mt1 = options.params.cross_section.params.multiplier;
    options.params.intra_section.params.multiplier = max(mt0, mt1);
    if ~isempty(z_wt)
        z_wt = z_wt(idxt);
    else
    ...
    if numel(options.mesh_space) > 1
        options.mesh_space = options.mesh_space(1);
    end
    % initialize mesh grid
    if options.share_M0
        refidx = max([1; find(fixed_sec(:))]);
        IMG = imread([imglist(refidx).folder, filesep, imglist(refidx).name]);
        M0 =  elastic_mesh.gen_eqtriang_mesh_mask(ones(size(IMG)), options.mesh_space, 1);
    end
    Ms = cell(Nimg,1);
            ...
            if fixed_sec(k)
                if isempty(M0t)
                    if exist([meshdir, filesep, strrep(imglist(k).name, options.ext,'.mat')], 'file')
                        ...
                        if ~options.share_M0
                            M0 =  elastic_mesh.gen_eqtriang_mesh_mask(maskw<255, options.mesh_space, 1);
                        end
    ...
    if Nelastic > 0
        ...
        cnt = 0;
        cnt_suc = 0;
        mt = ones(Nelastic, 1) * mt0;
        if options.Nsoft > 0 && mt1 > mt0
            mt(1:options.Nsoft) = linspace(mt1, mt0, options.Nsoft);
        end
        for k = 1:Nelastic
            t = t+1;
            % tic;
            options.params.intra_section.params.multiplier = mt(k);
