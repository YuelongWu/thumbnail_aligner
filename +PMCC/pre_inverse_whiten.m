function [img, mask] = pre_inverse_whiten(img, options, maskt)
    ...
    img = options.max_val - single(img);
    if options.thresh > 0
    ...
    if options.sigma > 0
        ...
        imgm = imgaussfilt(img, options.sigma);
        imgm2 = imgaussfilt(img.^2, options.sigma);
        imgv = (imgm2 - imgm.^2);
        imgv = max(imgv, mean(imgv(imgv>0)));
        img = (img - imgm)./imgv.^0.5;
    end
    img = img .* mask;
end
