function M = init_mesh_subregion_old(M0, mask_region, mask_wrinkle, options)
    if nargin < 4
        options = struct;
    end
    ...
    glue_idx = all(edg_id0 > 0, 2) & stiffness > 0 & range(edg_id0, 2) > 0;
    if any(glue_idx)
        glue_edg = edg0(glue_idx, :);
        glue_edg = repmat(glue_edg(:), 1, 2) + repmat(M.pt_num0 * (edg_id0(glue_idx,:) - 1), 2, 1);
